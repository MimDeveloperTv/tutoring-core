...
class SurgeryCalculationService
{
    public $globalData = [];
    public $test;
...
//            $this->globalData['age'] = $this->globalData['age'] > 0 ? $this->globalData['age'] : 1;
            $eye_variable = EyeVariable::where('patient_id',$id)->where('eye_type',$eye)->first();
            if(!$eye_variable)
                $eye_variable = EyeVariable::create([
                    'patient_id' => $id,
                    'eye_type' => $eye,
                    'updated_by' => 'optometry'
                ]);
            $this->globalData = $eye_variable->toArray();
...
//        return  response()->json($this->test);
    ...
    public function surgery_prk()
    {
        ...
        $conditions['Pupil Offset'] = [
            ['title' => 'x', 'value' => isset($this->globalData['PupilX']) ? $this->globalData['PupilX'] : null],
            ['title' => 'y', 'value' => isset($this->globalData['PupilY']) ? $this->globalData['PupilY'] : null]
        ];

        $conditions['Optical Zone'] = [
            ['title' => 'mm', 'value' => isset($this->globalData['optical_zone_mm']) ? $this->globalData['optical_zone_mm'] : null]
        ];

        $conditions['Epithelium Thickness'] = [
            ['title' => 'um', 'value' => 55]
        ];

        $conditions['Max Ablation'] = [
            ['title' => 'um', 'value' => isset($this->globalData['max_ablation_um']) ? $this->globalData['max_ablation_um'] : null, 'error' => isset($this->globalData['max_ablation_um']) && $this->globalData['max_ablation_um'] > 125 ? 1 : 0]
        ];

        $conditions['RSB'] = [
            ['title' => 'um', 'value' => (isset($this->globalData['max_ablation_um']) && isset($this->globalData['pachy_min'])) ? $this->globalData['pachy_min'] - $this->globalData['max_ablation_um'] : null, 'error' => isset($this->globalData['max_ablation_um']) && isset($this->globalData['pachy_min']) && ($this->globalData['pachy_min'] - $this->globalData['max_ablation_um']) < 400 ? 1 : 0],
        ];

        $conditions['MMC'] = [
            ['title' => 'sec', 'value' => isset($this->globalData['mmc_second']) ? $this->globalData['mmc_second'] : null]
        ];
    ...
    public function surgery_transprk()
    {
        ...
        $conditions['Diopter'] = [
            ['title' => 'sph', 'value' => isset($this->globalData['diopter_D_sph']) ? $this->globalData['diopter_D_sph'] : null, 'error' => !empty($this->globalData['diopter_D_sph']) && $this->globalData['diopter_D_sph'] >= 0 ? 1 : 0],
            ['title' => 'cyl', 'value' => isset($this->globalData['diopter_D_cyl']) ? $this->globalData['diopter_D_cyl'] : null],
            ['title' => 'axis', 'value' => isset($this->globalData['diopter_D_axis']) ? $this->globalData['diopter_D_axis'] : null]
        ];
        ...
        $conditions['Max Ablation'] = [
            [ 'title' => 'um', 'value' => isset($this->globalData['max_ablation_um']) ? $this->globalData['max_ablation_um'] + 55 : null, 'error' => isset($this->globalData['max_ablation_um']) && $this->globalData['max_ablation_um'] > 180 ? 1 : 0]
        ];

        $conditions['RSB'] = [
            ['title' => 'um', 'value' => isset($this->globalData['max_ablation_um']) && isset($this->globalData['pachy_min']) ? $this->globalData['pachy_min'] - $this->globalData['max_ablation_um'] - 55 : null, 'error' => isset($this->globalData['max_ablation_um']) && isset($this->globalData['pachy_min']) && ($this->globalData['pachy_min'] - $this->globalData['max_ablation_um'] - 55) < 345 ? 1 : 0]
        ];
    ...
    public function surgery_lasik()
    {
        ...
        $conditions['Max Ablation'] = [
            ['title' => 'um', 'value' => isset($this->globalData['max_ablation_um']) ? $this->globalData['max_ablation_um'] : null, 'error' => isset($this->globalData['max_ablation_um']) && $this->globalData['max_ablation_um'] > 140 ? 1 : 0]
        ];

        $conditions['RSB'] = [
            ['title' => 'um', 'value' => isset($this->globalData['max_ablation_um']) && isset($this->globalData['pachy_min']) ? $this->globalData['pachy_min'] - $this->globalData['max_ablation_um'] - 110 : null, 'error' => isset($this->globalData['max_ablation_um']) && isset($this->globalData['pachy_min']) && ($this->globalData['pachy_min'] - $this->globalData['max_ablation_um'] - 110) < 300 ? 1 : 0]
        ];

        $conditions['Flap Diameter'] = [
            ['title' => 'Flap Diameter', 'value' => isset($this->globalData['flap_diameter']) ? $this->globalData['flap_diameter'] : null]
        ];

        $conditions['Flap Thickness'] = [
            ['title' => 'um', 'value' => 110]
        ];

        $conditions['Hinge Length'] = [
            ['title' => 'Hinge Length', 'value' => 0.5]
        ];

        $conditions['Hinge Position'] = [
            ['title' => 'Hinge Position', 'value' => 'superior']
        ];

        $conditions['Side Cut Angle'] = [
            ['title' => 'Side Cut Angle', 'value' => 90]
        ];
    ...
    public function surgery_smile()
    {
        ...
        $conditions['Diopter'] = [
            ['title' => 'sph', 'value' => isset($this->globalData['diopter_D_sph']) ? $this->globalData['diopter_D_sph'] : null, 'error' => isset($this->globalData['diopter_D_sph']) && $this->globalData['diopter_D_sph'] >= 0 ? 1 : 0],
            ['title' => 'cyl', 'value' => isset($this->globalData['diopter_D_cyl']) ? $this->globalData['diopter_D_cyl'] : null],
            ['title' => 'axis', 'value' => isset($this->globalData['diopter_D_axis']) ? $this->globalData['diopter_D_axis'] : null]
        ];
        ...
        $conditions['Max Ablation'] = [
            [ 'title' => 'um', 'value' => isset($this->globalData['max_ablation_um']) ? $this->globalData['max_ablation_um'] : null, 'error' => isset($this->globalData['max_ablation_um']) && $this->globalData['max_ablation_um'] > 160 ? 1 : 0]
        ];

        $conditions['RSB'] = [
            ['title' => 'um', 'value' => isset($this->globalData['max_ablation_um']) && isset($this->globalData['pachy_min']) ? $this->globalData['pachy_min'] - $this->globalData['max_ablation_um'] - 135 : null, 'error' => isset($this->globalData['max_ablation_um'])
            && isset($this->globalData['pachy_min']) && ($this->globalData['pachy_min'] - $this->globalData['max_ablation_um'] - 135) < 270 ? 1 : 0]
        ];

        $conditions['Treatment Pack Size'] = [
            ['title' => 'Treatment Pack Size', 'value' => 'S (9.5)']
        ];

        $conditions['Lenticule Parameters'] = [
            ['title' => 'Transition Zone', 'value' => '0.1']
        ];

        $conditions['Lenticule Parameters'] = [
            ['title' => 'Min Thichkness', 'value' => '15'],
            [ 'title' => 'Side cut angle', 'value' => '100']
        ];

        $conditions['Cap Parameters'] = [
            ['title' => 'Diameter', 'value' => '7.7'],
            ['title' => 'Thichkness', 'value' => '120'],
            ['title' => 'Side cut angle', 'value' => '90']
        ];

        $conditions['Incision Parameters'] = [
            ['title' => 'Position', 'value' => '120/60']
        ];

        $conditions['Incision Parameters'] = [
            ['title' => 'Angle', 'value' => '45'],
            ['title' => 'Width', 'value' => '3']
        ];
    ...
    public function surgery_kcn_keratoplasty()
    {
        ...
        $conditions['WTW'] = [
            [ 'title' => 'WTW', 'value' => isset($this->globalData['eyeexam_examination_WTW']) ? $this->globalData['eyeexam_examination_WTW'] : null]
        ];

        $conditions['Diameter Host (mm)'] = [
            ['title' => 'mm', 'value' => isset($this->globalData['eyeexam_examination_WTW']) ? $this->globalData['eyeexam_examination_WTW'] : null]
        ];

        $conditions['Diameter Donor (mm)'] = [
            ['title' => 'mm', 'value' => isset($diameter_donor_mm) ? $diameter_donor_mm : null]
        ];

        $conditions['Tickness'] = [
            [ 'title' => 'Tickness', 'value' => isset($this->globalData['pachy_min']) ? $this->globalData['pachy_min'] : null]
        ];
    ...
    public function surgery_kcn_cxl()
    {
        ...
        $conditions['Pre-Op. Diagnosis'] = [
            ['title' => 'Pre-Op. Diagnosis', 'value' => ''],
            ['title' => 'Post-Op. Diagnosis', 'value' => '']
        ];

        $conditions['Kind of Operation'] = [
            ['title' => 'Kind of Operation', 'value' => '']
        ];

        $conditions['Procedure and Findings'] = [
            [ 'title' => 'Procedure and Findings', 'value' => '']
        ];

        $conditions['Irradiation intensity'] = [
            ['title' => 'Irradiation intensity', 'value' => '3mw/cm3 from 5cm']
        ];

        $conditions['Irradiation time'] = [
            [ 'title' => 'Irradiation time', 'value' => '30 min']
        ];

        $conditions['Riboflavin'] = [
            ['title' => 'Riboflavin', 'value' => '0.1%']
        ];

        $conditions['Drop'] = [
            ['title' => 'Drop', 'value' => 'q 3min×10']
        ];
    ...
    public function surgery_kcn_ring_keraring()
    {
        ...
        $conditions['ICRS'] = [
            ['title' => 'ICRS Thickness', 'value' => isset($getKeraringICRS) && is_array($getKeraringICRS) ? $getKeraringICRS[0] : null],
            ['title' => 'ICRS Thickness', 'value' => isset($getKeraringICRS) && is_array($getKeraringICRS) ? $getKeraringICRS[1] : null],
        ];

        $conditions['ICRS Arc Length (Degrees)'] = [
            ['title' => 'Arc Length (Degrees)', 'value' => '90-210']
        ];

        $conditions['Position'] = [
            ['title' => 'Position', 'value' => isset($this->globalData['position']) ? $this->globalData['position'] : null]
        ];

        $conditions['Depth in Cornea'] = [
            [ 'title' => 'um', 'value' => isset($this->globalData['depth_in_cornea']) ? $this->globalData['depth_in_cornea'] : null]
        ];

        $conditions['Inner Diameter'] = [
            ['title' => 'Inner Diameter', 'value' => 6]
        ];

        $conditions['Outer Diameter'] = [
            ['title' => 'Outer Diameter', 'value' => 7]
        ];
    ...
    public function surgery_kcn_ring_feraring()
    {
        ...
        if (isset($this->globalData['type']) && ($this->globalData['type'] == 1 || $this->globalData['type'] == 2)) {
            if (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) <= 2) {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => 'None/150'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 2 && abs($this->globalData['astig_topo']) <= 4) {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => 'None/200'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 4 && abs($this->globalData['astig_topo']) <= 6) {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => 'None/250'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 6 && abs($this->globalData['astig_topo']) <= 8) {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => 'None/300'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 8 && abs($this->globalData['astig_topo']) <= 10) {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => '150/250'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 10) {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => '200/300'];
            } else {
                $conditions['ICRS Thickness'][] = [ 'title' => 'ICRS Thickness', 'value' => ''];
            }
        } elseif (isset($this->globalData['type']) && $this->globalData['type'] == 3) {
            if (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) <= 2) {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => 'None/150'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 2 && abs($this->globalData['astig_topo']) <= 4) {
                $conditions['ICRS Thickness'][] = [ 'title' => 'ICRS Thickness', 'value' => '150/200'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 4 && abs($this->globalData['astig_topo']) <= 6) {
                $conditions['ICRS Thickness'][] = [ 'title' => 'ICRS Thickness', 'value' => '200/250'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 6) {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => '250/300'];
            } else {
                $conditions['ICRS Thickness'][] = ['title' => 'ICRS Thickness', 'value' => ''];
            }
        } elseif (isset($this->globalData['type']) && $this->globalData['type'] == 4) {
            if (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) <= 2) {
                $conditions['ICRS Thickness'][]= [ 'title' => 'ICRS Thickness', 'value' => '150/150'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 2 && abs($this->globalData['astig_topo']) <= 4) {
                $conditions['ICRS Thickness'][] = [ 'title' => 'ICRS Thickness', 'value' => '200/200'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 4 && abs($this->globalData['astig_topo']) <= 6) {
                $conditions['ICRS Thickness'][] = [ 'title' => 'ICRS Thickness', 'value' => '250/250'];
            } elseif (isset($this->globalData['astig_topo']) && abs($this->globalData['astig_topo']) > 6) {
                $conditions['ICRS Thickness'][] = [ 'title' => 'ICRS Thickness', 'value' => '300/300'];
            } else {
                $conditions['ICRS Thickness'][] = [ 'title' => 'ICRS Thickness', 'value' => ''];
            }
        } else {
        ...
        $conditions['ICRS Arc Length (Degrees)'] = [
            ['title' => 'Arc length', 'value' => '90-210']
        ];

        $conditions['Position']= [['title' => 'Position', 'value' => isset($this->globalData['position']) ? $this->globalData['position'] : null]];
        $conditions['Depth in Cornea']= [['title' => 'um', 'value' => isset($this->globalData['depth_in_cornea']) ? $this->globalData['depth_in_cornea'] : null]];
        $conditions['Inner Diameter'] =[['title' => 'Inner Diameter', 'value' => 4.8]];
        $conditions['Outer Diameter'] = [['title' => 'Outer Diameter', 'value' => 5.4]];
        $conditions['Arc']= [['title' => 'Arc', 'value' => 180]];
    ...
    public function surgery_kcn_ring_intacs()
    {
        ...
        if (isset($this->globalData['type']) && ($this->globalData['type'] == 3 || $this->globalData['type'] == 4)) {
            if (isset($this->globalData['subjective_ref_sph']) && $this->globalData['subjective_ref_sph'] <= 0.0 && $this->globalData['subjective_ref_sph'] > -1) {
                $conditions['ICRS Thickness'][] = [ 'title' => 'Inferior Intacs (um)', 'value' => 210];
                $conditions['ICRS Thickness'][] = [ 'title' => 'Superior Intacs (um)', 'value' => 210];
                ...
                $conditions['ICRS Thickness'][] = [ 'title' => 'Inferior Intacs (um)', 'value' => 250];
                $conditions['ICRS Thickness'][] = [ 'title' => 'Superior Intacs (um)', 'value' => 250];
                ...
                $conditions['ICRS Thickness'][] = [ 'title' => 'Inferior Intacs (um)', 'value' => 300];
                $conditions['ICRS Thickness'][] = [ 'title' => 'Superior Intacs (um)', 'value' => 300];
                ...
                $conditions['ICRS Thickness'][] = [ 'title' => 'Inferior Intacs (um)', 'value' => 350];
                $conditions['ICRS Thickness'][] = [ 'title' => 'Superior Intacs (um)', 'value' => 350];
                ...
                $conditions['ICRS Thickness'][] = [ 'title' => 'Inferior Intacs (um)', 'value' => 400];
                $conditions['ICRS Thickness'][] = [ 'title' => 'Superior Intacs (um)', 'value' => 400];
            } elseif (isset($this->globalData['subjective_ref_sph']) && $this->globalData['subjective_ref_sph'] <= -5) {
                $conditions['ICRS Thickness'][] = [ 'title' => 'Inferior Intacs (um)', 'value' => 450];
                $conditions['ICRS Thickness'][] = [ 'title' => 'Superior Intacs (um)', 'value' => 450];
            } else {
                $conditions['ICRS Thickness'][] = [ 'title' => 'Inferior Intacs (um)', 'value' => ''];
                $conditions['ICRS Thickness'][] = [ 'title' => 'Superior Intacs (um)', 'value' => ''];
            }
        ...
        $conditions['ICRS Arc Length (Degrees)']= [[ 'title' => 'Arc length', 'value' => 150]];
        $conditions['Position']= [['title' => 'Position', 'value' => isset($this->globalData['position']) ? $this->globalData['position'] : null]];
        $conditions['Depth in Cornea'] =[ ['title' => 'um', 'value' => isset($this->globalData['depth_in_cornea']) ? $this->globalData['depth_in_cornea'] : null]];
        $conditions['Inner Diameter'] = [['title' => 'Inner Diameter', 'value' => 6.77]];
        $conditions['Outer Diameter'][] = ['title' => 'Outer Diameter', 'value' => 8.1];
        $conditions['Arc'][] = [['title' => 'Arc', 'value' => 180]];
    ...
    public function surgery_kcn_ring_intacs_sk()
    {
        ...
        if (isset($this->globalData['subjective_ref_sph']) && isset($this->globalData['subjective_ref_cyl']) && isset($this->globalData['type'])) {
            ...
            if ($this->globalData['subjective_ref_sph'] < 0 && abs($this->globalData['subjective_ref_sph']) > abs($this->globalData['subjective_ref_cyl']) && ($this->globalData['type'] == 3 || $this->globalData['type'] == 4)) {
                if ($SEQ <= 0 && $SEQ > -1) {
                    $conditions['ICRS Thickness'] = [
                        [ 'title' => 'Ring 1', 'value' => 210],
                        [ 'title' => 'Ring 2', 'value' => 210]
                ];

                } elseif ($SEQ <= -1 && $SEQ > -2) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 250];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 250];
                } elseif ($SEQ <= -2 && $SEQ > -4) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 300];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 300];
                } elseif ($SEQ <= -4 && $SEQ > -6) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 350];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 350];
                } elseif ($SEQ <= -6 && $SEQ > -8) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 400];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 400];
                } elseif ($SEQ <= -8 && $SEQ > -10) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 450];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 450];
                } elseif ($SEQ <= -10) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 500];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 500];
                } else {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => ''];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => ''];
                }
            ...
            elseif (($this->globalData['subjective_ref_sph'] > 0 && $this->globalData['subjective_ref_sph'] <= 3) && ($this->globalData['type'] == 3 || $this->globalData['type'] == 4)) {
                ...
                $conditions['ICRS Arc Length (Degrees)'][] = [ 'title' => 'Arc length', 'value' => 90];
            ...
            elseif ($this->globalData['subjective_ref_sph'] < 0 && abs($this->globalData['subjective_ref_sph']) == abs($this->globalData['subjective_ref_cyl']) && ($this->globalData['type'] == 1 || $this->globalData['type'] == 2)) {
                if (($this->globalData['subjective_ref_sph'] <= 0 && $this->globalData['subjective_ref_sph'] > -1.25) && ($this->globalData['subjective_ref_cyl'] <= -2 && $this->globalData['subjective_ref_cyl'] > -3)) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 300];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 210];
                    $conditions['ICRS Arc Length (Degrees)'][] = [ 'title' => 'Arc length', 'value' => 150];
                    ...
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => ''];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => ''];
                    $conditions['ICRS Arc Length (Degrees)'][] = [ 'title' => 'Arc length', 'value' => ''];
                }
            } // sph > 0 & type 1 & 2
            ...
            elseif ($this->globalData['subjective_ref_sph'] < 0 && abs($this->globalData['subjective_ref_sph']) < abs($this->globalData['subjective_ref_cyl']) && ($this->globalData['type'] == 3 || $this->globalData['type'] == 4)) {
                if (($this->globalData['subjective_ref_sph'] <= 0 && $this->globalData['subjective_ref_sph'] > -1.75) && ($this->globalData['subjective_ref_cyl'] <= -3 && $this->globalData['subjective_ref_cyl'] > -5)) {
                    ...
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 350];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 350];
                    $conditions['ICRS Arc Length (Degrees)'][] = [ 'title' => 'Arc length', 'value' => 130];
            ...
            elseif ($this->globalData['subjective_ref_sph'] < 0 && abs($this->globalData['subjective_ref_sph']) == abs($this->globalData['subjective_ref_cyl']) && ($this->globalData['type'] == 3 || $this->globalData['type'] == 4)) {
                if ($SEQ <= 0 && $SEQ > -1) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 210];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 210];
                } elseif ($SEQ <= -1 && $SEQ > -2) {
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 1', 'value' => 250];
                    $conditions['ICRS Thickness'][] = [ 'title' => 'Ring 2', 'value' => 250];
        ...
        $conditions['Position'] = [['title' => 'Position', 'value' => isset($this->globalData['position']) ? $this->globalData['position'] : null]];
        $conditions['Depth in Cornea'] = [['title' => 'um', 'value' => isset($this->globalData['depth_in_cornea']) ? $this->globalData['depth_in_cornea'] : null]];
        $conditions['Inner Diameter'] = [['title' => 'Inner Diameter', 'value' => 6]];
        $conditions['Outer Diameter'] = [['title' => 'Outer Diameter', 'value' => 7]];
        $conditions['Arc'] = [['title' => 'Arc', 'value' => 180]];
    ...
    public function surgery_kcn_ring_myoring()
    {
        ...
        if (isset($this->globalData['KM_D']) && $this->globalData['KM_D'] < 44) {
            $conditions['Implant diameter(mm)']=[['title' => 'Implant diameter(mm)', 'value' => 7]];
            $conditions['Implant thickness(um)'] = [['title' => 'Implant thickness(um)', 'value' => 280]];
        } elseif (isset($this->globalData['KM_D']) && 44 <= $this->globalData['KM_D'] && $this->globalData['KM_D'] < 48) {
            $conditions['Implant diameter(mm)'] = [['title' => 'Implant diameter(mm)', 'value' => 6]];
            $conditions[ 'Implant thickness(um)'] = [['title' => 'Implant thickness(um)', 'value' => 240]];
            ...
            $conditions['Implant diameter(mm)'] = [[ 'title' => 'Implant diameter(mm)', 'value' => 5]];
            $conditions['Implant thickness(um)'] = [['title' => 'Implant thickness(um)', 'value' => 280]];
        } elseif (isset($this->globalData['KM_D']) && $this->globalData['KM_D'] >= 55) {
            $conditions['Implant diameter(mm)'] = [[ 'title' => 'Implant diameter(mm)', 'value' => 5]];
            $conditions['Implant thickness(um)'] = [['title' => 'Implant thickness(um)', 'value' => 320]];
        } else {
            $conditions['Implant diameter(mm)'] = [[ 'title' => 'Implant diameter(mm)', 'value' => '']];
            $conditions['Implant thickness(um)'] = [['title' => 'Implant thickness(um)', 'value' => '']];
        }

        $conditions['Position'] = [['title' => 'Position', 'value' => isset($this->globalData['position']) ? $this->globalData['position'] : null]];
        $conditions['Depth in Cornea'] = [['title' => 'um', 'value' => isset($this->globalData['depth_in_cornea']) ? $this->globalData['depth_in_cornea'] : null]];
        $conditions['ICRS Arc Length (Degrees)'] = [['title' => 'Arc length', 'value' => 360]];
        $conditions['Inner Diameter'] = [['title' => 'Inner Diameter', 'value' => 5]];
        $conditions['Outer Diameter'] = [['title' => 'Outer Diameter', 'value' => 8]];
